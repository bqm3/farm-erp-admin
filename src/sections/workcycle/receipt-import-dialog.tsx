...
import { listPartners } from 'src/api/partners'; // ✅ cần có api này
...
export default function ReceiptImportDialog({ open, onClose, workCycleId, onSubmit }: Props) {
  ...
  const canSubmit = useMemo(() => {
    ...
    return true;
  }, [workCycleId, receipt_date, fund_id, speciesId, qty, totalBeforeTax, vatPercent]);
  ...
  const fetchPartners = useCallback(async () => {
    try {
      // bạn có thể truyền q/page/limit tuỳ API
      const res: any = await listPartners({ page: 1, limit: 200 });
      const rows: PartnerOpt[] = res?.rows ?? res?.data ?? [];
      setPartners(rows);
    } catch (e: any) {
      // partner optional => fail cũng không chặn
      enqueueSnackbar(e?.message || 'Không load được nhà cung cấp', { variant: 'warning' });
      setPartners([]);
    }
  }, []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const desc =
        (description || '').trim() || `Nhập giống${speciesName ? ` - ${speciesName}` : ''}`;

      const unitPrice = n(qty, 0) > 0 ? n(totalBeforeTax, 0) / n(qty, 0) : 0;
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <TextField label="Loại" value="Chi" fullWidth disabled />
            <TextField label="Loại phụ" value="Thêm" fullWidth disabled />
          </Stack>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <TextField label="Tiền VAT" value={computed.vat_amount} fullWidth disabled />
            <TextField
